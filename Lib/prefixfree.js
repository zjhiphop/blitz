 ...
 * @author Lea Verou
 * MIT license
 * https://github.com/zjhiphop/prefixfree
 */
...
(function(head) {
    ...
    var self = window.PrefixFree = {
        ...
        process : {
            link : function(link) {
                ...
                var url = link.getAttribute('href') || link.getAttribute('data-href'), parent = link.parentNode, xhr = new XMLHttpRequest();
    ...
    (function() {
        ...
        // Why are we doing this instead of iterating over properties in a .style
        // object? Cause Webkit won't iterate over those.
        var iterate = function(property) {
            ...
            if(property.indexOf('-') > -1) {
                ...
                if(property.charAt(0) === '-') {
                    var prefix = parts[1], uses = ++prefixes[prefix] || 1;
                    ...
                    while(parts.length > 3) {
                        ...
                        var shorthand = parts.join('-'), shorthandDOM = camelCase(shorthand);
        ...
        for(var i = 0; i < properties.length; i++) {
            ...
            if(property.charAt(0) !== '-') {
                break;
                // it's sorted, so once we get to the first unprefixed property,
                // we're done
            }

            if(property.indexOf(self.prefix) === 0) {// we might have multiple
                // prefixes, like Opera
    ...
    (function() {
        ...
        values['repeating-linear-gradient'] = values['repeating-radial-gradient'] = values['radial-gradient'] = values['linear-gradient'];
        ...
        for(var val in values) {
            ...
            var test = values[val], property = test.property, value = val + (test.params ? '(' + test.params + ')' : '');
    ...
    (function() {
        ...
        for(var selector in selectors) {
            var rule = selectors[selector] ? '(' + selectors[selector] + '){}' : '{}', test = self.prefixSelector(selector) + rule + selector + rule;
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if(window.CSSStyleDeclaration) {
            for(var i = 0; i < self.properties.length; i++) {
                var property = camelCase(self.properties[i]), prefixed = camelCase(self.prefix + property), proto = CSSStyleDeclaration.prototype, getter = (function(prefixed) {
                    return function() {
                        return this[prefixed];
                    }
                })(prefixed), setter = (function(prefixed) {
